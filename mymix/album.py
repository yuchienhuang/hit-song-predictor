...
def hit_song_predictor(album, model):
    ...
    indices = np.array(heapq.nlargest(2, y_est_prob_mtx, key=lambda x: x[1]))[:,0].astype(int)

    top_two = album.tracks_df[[ 'track_number',  'name']].values[indices]

    return hit_list, top_two
